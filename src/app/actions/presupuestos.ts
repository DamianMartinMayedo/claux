...
export async function listarPresupuestos(): Promise<PresupuestoRow[]> {
  ...
  const { data } = await db
    ...
    .select('id, created_at, comercial_nombre, nombre_negocio, tarifa, horas_total, coste_instalacion_usd, cuota_mensual_usd, horas_reales, estado')
...
export async function actualizarHorasReales(
  ...
  const { error } = await db
    .from('presupuestos_instalacion')
    .update({ horas_reales: valor, estado: valor != null ? 'instalado' : 'guardado', updated_at: new Date().toISOString() })
    .eq('id', id)
  if (error) return { ok: false, error: error.message }
  revalidatePath('/admin/presupuestos')
  return { ok: true }
