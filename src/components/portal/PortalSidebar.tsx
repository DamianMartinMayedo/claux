...
export default function PortalSidebar({ modulosVisibles, catalogo, catalogoEtiqueta, suscripcionEtiqueta, catalogoIcono }: Props) {
  ...
  return (
    ...
    {showLogoutDialog && (
      <ConfirmDialog title="Cerrar sesión" body="¿Cerrar sesión?" confirmLabel="Cerrar sesión" pendingLabel="Cerrando sesión…" pending={pending} danger onConfirm={confirmLogout} onCancel={() => setShowLogoutDialog(false)} />
    )}
  </>
  )
}
