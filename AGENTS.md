...
>
> **Acciones de fila**: si una fila tiene **2+ acciones**, usa un único menú `⋯` con el componente `RowActions` (`src/components/portal/RowActions.tsx`) — no pongas una fila de botones-icono (se amontonan y recortan). Sus items son `<button className="row-actions-item">` o `<Link className="row-actions-item">` (+ `-danger` / `-success`) con icono **y** texto. El menú se posiciona en `fixed` para escapar del `overflow` de `.card-table`, y ya hace `stopPropagation`. Los enlaces heredan el color del texto por defecto; solo las acciones negativas (`-danger`) llevan color rojo. Con **1 sola acción**, deja el icono directo (`.ter-action-btn`). Referencia: `GastosView.tsx`, `TercerosView.tsx`.
>
