...
Inventario depende de la base, nunca a la inversa (su selector de productos ya era la conveniencia que la base aprovecha en las líneas de venta). Migraciones 035–037. La mutación de stock y confirmar/anular compra viven en **funciones Postgres atómicas** (037: `inv_aplicar_movimiento`, `inv_confirmar_compra`, `inv_anular_compra`): una sola transacción (rollback total si algo falla) con incrementos atómicos (sin leer-modificar-escribir, seguro ante concurrencia); los servicios en las líneas no generan stock. `inv_recalcular_stock` reconstruye el stock desde el ledger (botón «Recalcular stock» en Movimientos) como red de seguridad.
