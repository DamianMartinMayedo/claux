...
- **`agenda` → "Citas"** (modo *agenda*): 1 cita por recurso/profesional y slot, con servicios de duración. Peluquerías, clínicas, alquileres. **Capa de datos lista y verificada** (tablas `recursos`/`servicios`/`recurso_servicios`/`recurso_horarios`; reutiliza `reservas` con `recurso_id`/`servicio_id`; RPC atómica `res_crear_cita`). **Pendiente la UI** (portal `/portal/citas`, público `/[slug]/citas`, rama del bot).
...
| Citas (agenda) + bot Telegram (`agenda`) | Agenda por recurso/profesional (1 cita por recurso y slot, servicios con duración): pensado para peluquerías, clínicas, alquileres. Comparte bot y notificaciones con Reservas. *(Capa de datos lista; UI en construcción.)* | +$10 / +$18 |
...
6. ~~Reservas: formulario público + panel en el portal + notificación Telegram al dueño.~~ **(HECHO — modo aforo `reservas_citas`, endurecido; ver §2.)** Pendiente: UI del módulo Citas (`agenda`, modo agenda) — capa de datos y RPC ya listas.
