...
/**
 * Cerrado no monta nada; abierto monta el formulario DE CERO, y así los valores de
 * precarga entran por el `useState` inicial de cada campo.
 *
 * Antes el componente vivía siempre montado y un efecto reiniciaba ocho estados al
 * abrir. Costaba un render de más y traía una trampa: el efecto dependía de
 * `initial`, un objeto; el día que un padre lo escribiera en el JSX en vez de
 * guardarlo en estado, se recrearía en cada render y borraría lo que el comercial
 * estuviera tecleando.
 */
export default function ClienteFormModal(props: Props) {
  if (!props.open) return null
  return <Formulario {...props} />
}

function Formulario({
  onClose, catalogo, plantillas, nombresNivel, descuentoAnualPct, parametros, initial, presupuestoId,
}: Props) {
  ...
  const [seleccionados, setSeleccionados] = useState<string[]>(initial?.modulos ?? [])
  const [sector, setSector] = useState(initial?.sector ?? '')
  const [nivel, setNivel] = useState<Nivel>(initial?.nivel ?? 'inicial')
  const [ciclo, setCiclo]   = useState<'mensual' | 'anual'>(initial?.ciclo ?? 'mensual')
  ...
  const [pagoSetup, setPagoSetup] = useState(initial?.pago_setup != null ? String(initial.pago_setup) : '')
  const [moneda, setMoneda] = useState<MonedaClaux>(initial?.moneda ?? 'USD')
  ...
  // Dos pantallas en el mismo diálogo: el formulario y, hecha el alta, las
  // credenciales con las que el cliente entra la primera vez. Comparten shell, y
  // con ella el Escape, el foco atrapado y el bloqueo del scroll del fondo —que
  // este modal, escrito a mano, no tenía por ningún lado.
  return (
    <ModalShell
