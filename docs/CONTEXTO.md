...
**Módulo RRHH construido (Fase 5):** Personal (tabla `empleados`: datos personales, contrato —tipo, fecha de alta, salario base, moneda, periodicidad, turno simple— y baja con `fecha_baja`/`motivo_baja`; estado ACTIVO/BAJA derivado de `fecha_baja`, no almacenado) + Nómina simple (`nominas` con estado BORRADOR/CONFIRMADA + `nomina_lineas`: una línea por empleado con devengado/deducciones/neto y snapshot de nombre/cargo). **Integración con la base contable:** al CONFIRMAR una nómina se registra automáticamente un GASTO «Salarios» en `gastos_cobros` (monto = Σ netos), que fluye a CxP, Tesorería y Reportes por la liquidación unificada existente; la nómina guarda `gasto_id` y deja de ser editable. RRHH depende de la base, nunca a la inversa. UI en `/portal/rrhh` con pestañas Personal | Nómina (`rrhh.ts` + `RrhhView`). Pendiente del módulo: planificador de turnos real (hoy es un campo de texto). Migraciones 027–028.
