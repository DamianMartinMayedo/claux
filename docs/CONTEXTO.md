...
**Módulo RRHH construido (Fase 5):** en `/portal/rrhh`, cinco pestañas (`rrhh.ts` + `RrhhView` orquestando `ContratosTab`/`TurnosTab`/`ReportesTab`):
- **Personal** (`empleados`): datos personales + snapshot de contrato (tipo, fecha de alta, salario base, moneda, periodicidad) y baja (`fecha_baja`/`motivo_baja`); estado ACTIVO/BAJA derivado de `fecha_baja`. El snapshot es lo que lee la nómina.
- **Contratos** (`contratos`): historial por empleado; "nuevo contrato" cierra el vigente, abre el nuevo y actualiza el snapshot del empleado. Editar el salario en Personal es una corrección rápida sin historial. Cada empleado nace con un contrato inicial.
- **Turnos** (`turnos` + `turno_asignaciones`): catálogo de turnos por empresa (nombre, horario, color) + planificador semanal recurrente (un turno por empleado y día, único por `(empleado, día)`).
- **Nómina simple** (`nominas` BORRADOR/CONFIRMADA + `nomina_lineas`): devengado/deducciones/neto por empleado. **Integración con la base contable:** al CONFIRMAR se registra un GASTO «Salarios» en `gastos_cobros` (monto = Σ netos) que fluye a CxP, Tesorería y Reportes por la liquidación unificada; la nómina guarda `gasto_id` y deja de ser editable.
- **Reportes**: analítica en cliente (sin tabla) sobre empleados+nóminas: plantilla activa, altas/bajas del año, coste de personal por moneda/mes/departamento.

RRHH depende de la base, nunca a la inversa. Migraciones 027–030.
