...
**Base contable en construcción (Fase 4):** **Tesorería** ya está implementada (cuentas caja/banco/pasarela, movimientos ingreso/egreso, transferencias misma-moneda, saldos por moneda — tablas `cuentas` + `movimientos_tesoreria`). Pendientes (placeholders en navegación): gastos/cobros, cuentas por cobrar/pagar y reportes financieros — todos en la **base contable** (ver §5). Compras y movimientos quedan en el **módulo inventario** (que ya tiene almacenes y productos); RRHH y asistente IA son módulos. El placeholder "contabilidad" (`modulo_contable`) se retira del MVP: la base ya cubre la contabilidad simple completa y un tier contable avanzado (partida doble / plan de cuentas oficial) es trabajo futuro.
...
3. **Completar la base contable (en curso, por tandas):** ~~gate del selector de productos por módulo Inventario~~ **(HECHO)** · ~~Tesorería~~ **(HECHO)** (cuentas, movimientos, transferencias, saldos por moneda) · **siguiente:** Gastos/Cobros, luego Cobros/Pagos con CxC/CxP (pagos parciales), luego Reportes financieros — reutilizando los patrones de Ventas y de Tesorería.
