...
**Base contable en construcción (Fase 4):** ya implementadas — **Tesorería** (cuentas caja/banco/pasarela, movimientos ingreso/egreso, transferencias misma-moneda, saldos por moneda — `cuentas` + `movimientos_tesoreria`) y **Gastos y cobros** (registro simple ingreso/egreso no facturado — tabla `gastos_cobros`; la liquidación es un movimiento de Tesorería con `origen=PAGO/COBRO` y `referencia_id`, admite pagos **parciales**; `monto_liquidado` y estado PENDIENTE/PARCIAL/LIQUIDADO se derivan). Pendientes (placeholders en navegación): cuentas por cobrar/pagar y reportes financieros — en la **base contable** (ver §5). Compras y movimientos quedan en el **módulo inventario** (que ya tiene almacenes y productos); RRHH y asistente IA son módulos. El placeholder "contabilidad" (`modulo_contable`) se retira del MVP: la base ya cubre la contabilidad simple completa y un tier contable avanzado (partida doble / plan de cuentas oficial) es trabajo futuro.
...
3. **Completar la base contable (en curso, por tandas):** ~~gate del selector de productos por módulo Inventario~~ **(HECHO)** · ~~Tesorería~~ **(HECHO)** · ~~Gastos y cobros~~ **(HECHO)** (registro simple + liquidación parcial vía Tesorería) · **siguiente:** CxC/CxP (aging de gastos/cobros y facturas pendientes) + cobro de facturas de Ventas reutilizando la liquidación, luego Reportes financieros — reutilizando los patrones de Ventas y de Tesorería.
