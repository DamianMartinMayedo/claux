...
- **Deuda de UI pendiente (ola 2):** migrar a clases del design system el resto de estilos inline con `var(--*)` (funcionan, pero la skill prefiere clases); quedan ~15 warnings de variables sin usar y 2 `<img>` por migrar a `next/image`. No hay bugs visibles pendientes conocidos.
